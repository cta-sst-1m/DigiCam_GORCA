...
void plotBiasCurve(string DataDir, const Float_t MaxCoincidentalRate=10, const Int_t GroupMultliplicity=2, const Int_t iMaxTelescopeMultiplicity = 2, Float_t fCoincidence = 50, Int_t iNumTelNextNeighborGroup =8, Int_t samples=100)
{
  ...
  */
  ...
  for(Int_t p = 0; p<NumPoints; p++)
    {
      BiasCurveValues[p] = BiasCurveValues[p]*samples/NumFiles;
      if( BiasCurveError[p]>0 )
	BiasCurveError[p] =  sqrt(BiasCurveError[p]*samples)/NumFiles;

      GroupRateVsThreshold[p] = GroupRateVsThreshold[p]*samples/NumFiles;
      if( GroupRateVsThresholdError[p]>0 )
	GroupRateVsThresholdError[p] =  sqrt(GroupRateVsThresholdError[p]*samples)/NumFiles;
  ...
  h->SetStats(0);
  h->Draw();
  TLine *lMaxAccidentals = new TLine( BiasCurveScanPoints.Min()*0.95 , MaxCoincidentalRate , BiasCurveScanPoints[lowestBin]*1.1, MaxCoincidentalRate);
  ...
  legend->AddEntry(gr,Form("%i group%s trigger; 1 telescope",GroupMultliplicity,GroupMultliplicity>1 ? "s" : ""),"pl");
  ...
  for(Int_t m =2;m<=iMaxTelescopeMultiplicity;m++)
    {
      ...
      label.Form("%i groups trigger; %i telescopes triggered",GroupMultliplicity,m);
