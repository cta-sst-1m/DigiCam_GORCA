...
* TLLOC   1       0.0       0.0       0.0  2.15  0.0  0.0
"""
...
arraystruct = [('len(telescopes)', 'SC', 0)]
...
#AP: Set according to the TDR (23Mar2015)""",
           ["""
* MIROR   {telID:>2} 4.306 2.15   0.0 2 18
"""],
...
if __name__ == '__main__':
    ...
    for ifactory, factory_def in enumerate(arraystruct):
        ...
        for tel in telescopes[start:start + ntels]:
            ...
            tel = dict(telID=int(telID), x=float(x), y=float(x), z=float(z), xoffset=float(xoffset),
                       yoffset=float(yoffset), **factory)
    ...
    with open(os.path.join(output, 'telescopes.mirrors'), 'w') as f_mirrors:
        f_mirrors.write('\n'.join(
            [mirrors[0], mirror_desings, reflectivities[0], mirror_reflectivities, mirrors[2], mirror_elements]))
